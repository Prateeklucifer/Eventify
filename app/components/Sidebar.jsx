    ...
    return (
        <nav className="py-3 md:py-4 px-4">
            <div className="container m-auto flex items-center justify-start md:justify-between gap-6">
                ...
                <div id="desk" className="hidden md:flex w-1/4 lg:w-1/5 xl:w-1/6 h-full top-0 left-0 px-6 py-4 fixed flex-col gap-6 bg-gray-100">
                    <Link href={"/"} className={`${sourceCodePro.className} logo font-semibold text-xl lg:text-2xl text-fuchsia-500 tracking-tight`}>Eventify</Link>
                    <ul className="flex flex-col gap-2 md:text-sm lg:text-base">
                        {links.map((link, key) => (
                            <li key={key}>
                                <Link href={link.href} className={`text-zinc-600 py-3 px-4 hover:bg-gray-200 hover:text-zinc-900 block rounded-md ${link.href == pathname && "font-bold bg-gray-200"}`} >{link.name}</Link>
                            </li>
                        ))}
                    </ul>
                </div>
